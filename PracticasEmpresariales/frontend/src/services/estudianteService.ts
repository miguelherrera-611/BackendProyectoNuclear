...
export const estudianteService = {
  async listar(estado?: EstadoEstudiante, page = 0, size = 20): Promise<Pageable<UsuarioResponse>> {
    ...
    const res = await api.get<ApiResponse<Pageable<UsuarioResponse>>>('/api/v1/estudiantes', { params })
    return res.data.datos!
  },

  async obtener(id: number): Promise<UsuarioResponse> {
    const res = await api.get<ApiResponse<UsuarioResponse>>(`/api/v1/estudiantes/${id}`)
    return res.data.datos!
  },

  async marcarApto(id: number, catalogoPracticaId: number): Promise<UsuarioResponse> {
    const res = await api.patch<ApiResponse<UsuarioResponse>>(`/api/v1/estudiantes/${id}/marcar-apto`, { catalogoPracticaId })
    return res.data.datos!
  },

  async mantenerNoApto(id: number, motivo: string): Promise<UsuarioResponse> {
    const res = await api.patch<ApiResponse<UsuarioResponse>>(`/api/v1/estudiantes/${id}/mantener-no-apto`, { motivo })
    return res.data.datos!
  },
