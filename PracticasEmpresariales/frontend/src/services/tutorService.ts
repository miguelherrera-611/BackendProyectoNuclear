...
export const tutorService = {
  async listarPorEmpresa(empresaId: number): Promise<TutorEmpresarialResponse[]> {
    const res = await api.get<ApiResponse<TutorEmpresarialResponse[]>>(`/v1/tutores/empresa/${empresaId}`)
    return res.data.datos ?? []
  },

  async crear(data: CrearTutorRequest): Promise<TutorEmpresarialResponse> {
    const res = await api.post<ApiResponse<TutorEmpresarialResponse>>('/v1/tutores', data)
    return res.data.datos!
  },

  async desactivar(id: number): Promise<TutorEmpresarialResponse> {
    const res = await api.patch<ApiResponse<TutorEmpresarialResponse>>(`/v1/tutores/${id}/desactivar`)
    return res.data.datos!
  },
}
